...
import setproctitle
import dataset
from model import IMBR, IMBR_Info
...
def main():
    ...
    for DIR in CONFIG['test']:
        ...
        for i in range(len(d)):
            ...
            # model
            if CONFIG['model'] == 'IMBR':
                graph = [ub_graph, ui_graph, bi_graph]
                info = IMBR_Info(64, dd['embed_L2_norm'], dd['mess_dropout'], dd['node_dropout'], 2)
                model = IMBR(info, assist_data, graph, device, pretrain=None).to(device)
